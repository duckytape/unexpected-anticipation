...
requirejs.config({
    ...
    paths: {
        ...
        HatrackActions: 'present/entryway',
        StickActions: 'present/stick'
    }
});

requirejs(['Texty', 'TCPConnection', 'OrchestrateStore', 'fs', 'HatrackActions', 'StickActions'],
function (Texty, TCPConnection, OrchestrateStore, fs, hatrackActions, stickActions) {

    // Load the game module and template (This should come in via RequireJS)
    var world = JSON.parse(fs.readFileSync(__dirname + '/present/world.json')),
